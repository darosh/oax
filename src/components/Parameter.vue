<template lang="pug">
  div
    ...
    v-text-field(v-model="value" v-else-if="!type && item.schema" multi-line :rows="3", :label="item.name", :hint="item.description", :required="item.required" persistent-hint)
    ...
    v-select(v-model="value" v-else-if="(type === 'array')", :items="item.items.enum" multiple :label="item.name", :hint="item.description", :required="item.required" persistent-hint)
    v-select(v-model="value" v-else-if="item.enum", :items="item.enum", :label="item.name", :hint="item.description", :required="item.required" persistent-hint)
    div(v-else) {{item}}
</template>
